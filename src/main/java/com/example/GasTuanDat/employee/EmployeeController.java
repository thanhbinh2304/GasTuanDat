...
public class EmployeeController {
    ...
    public ApiResponse<List<EmployeeResponse>> getAll() {
        ...
        public ApiResponse<PageResult<EmployeeResponse>> searchEmployees(
            @RequestParam(required = false) String keyword,
            @RequestParam(required = false) String jobTitle,
            @RequestParam(defaultValue = "1", required = false) int page,
            @RequestParam(defaultValue = "15", required = false) int limit
        ) {

        PageResult<EmployeeResponse> result = employeeService.search(keyword, jobTitle, page, limit);

        return ApiResponse.<PageResult<EmployeeResponse>>builder()
            .code(200)
            .message("Search employees success")
            .data(result)
            .build();
        }

    @GetMapping("/code/{employeeCode}")
    @Operation(summary = "Get employee by employeeCode")
    public ApiResponse<EmployeeResponse> getByEmployeeCode(@PathVariable String employeeCode) {
        return ApiResponse.<EmployeeResponse>builder()
                .code(200)
                .message("Get employee success")
                .data(employeeMapper.toResponse(employeeService.getByEmployeeCode(employeeCode)))
                .build();
    }
