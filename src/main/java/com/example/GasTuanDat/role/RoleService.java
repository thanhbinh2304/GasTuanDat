...
public class RoleService {
    ...
    public RoleEntity create(RoleCreationRequest request) {
        RoleEntity role = RoleEntity.builder()
                .roleName(canonicalizeRoleName(request.getRoleName()))
    ...
    public RoleEntity getByName(String roleName) {
        return roleRepository.findByRoleName(canonicalizeRoleName(roleName))
                .orElseThrow(() -> new ApiException(ErrorCode.ROLE_NOT_FOUND));
    }

    public RoleEntity update(UUID roleId, RoleUpdateRequest request) {
        RoleEntity role = getById(roleId);
        role.setRoleName(canonicalizeRoleName(request.getRoleName()));
    ...
    private String canonicalizeRoleName(String roleName) {
